        ...
        result.forEach((res: any) => {
          res.product_changes = res.product_changes.filter((change: any) => {
            return (
              ...
              change.createdAt.toLocaleDateString("pt-Pt") ==
              date
            );
          });
        });
...
/*IT WORKS         pasas la id del producto e ya*/ 
  ...
  /*IT WORKS pasas el nombre de la categoria y ya*/async  getPriceHistoryByCategory(category: any): Promise<any> {
    let categoryid= await Category.getOne({name:category}) as any
    return productModel
    .getAllPopulated(
      {category:categoryid._id},
